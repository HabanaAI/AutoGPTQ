...
class QuantLinear(nn.Module):
    ...
    def __init__(
        ...
        self.use_hpu_fp16 = use_cuda_fp16 if bits != 8 else False
    ...
    def forward(self, x):
        ...
        if False:
            ...
            if self.bits in [4]:
                ...
                weight = torch.ops.hpu.convert_from_int4(self.qweight, self.scales, self.qzeros, out_dtype=torch.int16 if self.bits == 8 else torch.int8)
            elif self.bits in [2, 8]:
                zeros = torch.bitwise_right_shift(
                    torch.unsqueeze(self.qzeros, 2).expand(-1, -1, 32 // self.bits),
                    self.wf.unsqueeze(0),
                ).to(torch.int16 if self.bits == 8 else torch.int8)
